...
class ObstacleStopper:
    ...
    def obstacles_callback(self, obstacles_msg):
        with self.lock:
            ...
            if self.check_obstacles(obstacles_msg, self.is_within_front_bounding_box):
                if self.linear_velocity > 0.01:
                    self.stop_robot()
            # elif self.check_obstacles(obstacles_msg, self.is_within_back_bounding_box):
            #     if self.linear_velocity < -0.01:
            #         self.stop_robot()
            # elif self.check_obstacles(obstacles_msg, self.still_front_bounding_box) or self.check_obstacles(obstacles_msg, self.still_back_bounding_box):
            #     if -0.01 <= self.linear_velocity <= 0.01:
            #         self.stop_robot()
            else:
                return
